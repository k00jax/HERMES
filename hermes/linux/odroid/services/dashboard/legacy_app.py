...
from dashboard.db.connect import open_db as db_open_db
from dashboard.db.events import ensure_events_table as db_ensure_events_table
from dashboard.db.queries import cutoff_iso as db_cutoff_iso
from dashboard.db.queries import parse_iso8601_utc as db_parse_iso8601_utc
from dashboard.db.queries import parse_local_datetime_input as db_parse_local_datetime_input
from dashboard.db.queries import query_series_points as db_query_series_points
from dashboard.db.queries import resolve_range_utc_from_request as db_resolve_range_utc_from_request
from dashboard.db.radar import summarize_radar_calibration_window as db_summarize_radar_calibration_window
from dashboard.db.reports import ensure_reports_table as db_ensure_reports_table
from dashboard.db.settings import ensure_settings_table as db_ensure_settings_table
from dashboard.db.settings import get_settings_payload as db_get_settings_payload
from dashboard.db.settings import reset_settings_payload as db_reset_settings_payload
from dashboard.db.settings import save_settings_payload as db_save_settings_payload
...
def detect_camera() -> bool:
    return bool(resolve_camera_device())
...
def read_latest_camera_meta() -> Dict[str, object]:
  try:
    text = Path(META_LATEST).read_text(encoding="utf-8")
...
def capture_snapshot(reason: str, extra: Optional[Dict[str, object]] = None) -> Dict[str, object]:
  ...
  try:
    ...
    return {"ok": False, "error": "snapshot_dir_unavailable", "detail": str(exc)}
  ...
  snap_path = Path(SNAP_LATEST)
  if not snap_path.exists() or snap_path.stat().st_size <= 0:
    return {"ok": False, "error": "capture_failed"}

  extra_payload: Dict[str, object] = {}
  if isinstance(extra, dict):
    extra_payload = {str(k): v for k, v in extra.items()}
  ...
  try:
    Path(META_LATEST).write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
  except Exception as exc:
    result["meta_write_error"] = str(exc)
  return result
  ...
  try:
    ...
    while True:
      ...
      try:
        ...
        ready = []
      if not ready:
        if (time.monotonic() - last_frame_ts) >= CAMERA_STREAM_STALL_SECS:
          LOGGER.warning("camera stream stalled for %.1fs; stopping stream", CAMERA_STREAM_STALL_SECS)
          break
        continue
      try:
        chunk = os.read(process.stdout.fileno(), CAMERA_STREAM_CHUNK_BYTES)
...
def open_db() -> sqlite3.Connection:
  return db_open_db(DB_PATH, DB_TIMEOUT_SECS)


def parse_iso8601_utc(raw: str) -> datetime.datetime:
  return db_parse_iso8601_utc(raw)
...
def get_settings_payload(conn: sqlite3.Connection) -> Dict[str, object]:
  return db_get_settings_payload(conn, SETTINGS_DEFAULTS, VALID_CHIME_KEYS)


def save_settings_payload(conn: sqlite3.Connection, updates: Dict[str, object]) -> Dict[str, object]:
  return db_save_settings_payload(
    conn,
    updates,
    SETTINGS_DEFAULTS,
    VALID_CHIME_KEYS,
...
def reset_settings_payload(conn: sqlite3.Connection) -> Dict[str, object]:
  return db_reset_settings_payload(conn, SETTINGS_DEFAULTS, now_utc_iso())
...
def parse_local_datetime_input(value: str) -> datetime.datetime:
  return db_parse_local_datetime_input(value)
...
def resolve_range_utc_from_request(
  ...
  try:
    return db_resolve_range_utc_from_request(
      preset=preset,
...
def summarize_radar_calibration_window(
  ...
  return db_summarize_radar_calibration_window(
    conn,
...
def query_series(series: str, minutes: int) -> List[Dict[str, object]]:
    ...
    try:
      return db_query_series_points(
        db_path=DB_PATH,
        timeout_secs=DB_TIMEOUT_SECS,
...
def camera_latest_meta() -> Dict[str, object]:
  ...
  payload = read_latest_camera_meta()
  if not payload:
    return {"ok": False, "error": "no_snapshot"}
  return payload
...
<head>
  ...
  <style>
    ...
    .target-conf { opacity: 0.75; }
    .target-bars { display: flex; flex-direction: column; gap: 6px; }
    .tbar { display: grid; grid-template-columns: 52px 1fr 36px; gap: 8px; align-items: center; }
    .tbar-label { opacity: 0.75; font-size: 11px; }
    ...
    .radar-evidence-label { font-size: 11px; color: #8ea1b3; margin-bottom: -2px; }
    ...
    .home2-vision-card .camera-stream { flex: 1 1 auto; width: 100%; aspect-ratio: 16 / 9; height: auto; max-height: none; min-height: 320px; }
    ...
    .hp-card .status-pill { margin-top: 8px; }
    ...
    .camera-stream { margin-top: 10px; width: 100%; object-fit: contain; background: #0b1118; border: 1px solid #26313d; border-radius: 8px; }
  </style>
</head>
<body>
...
def render_settings_page() -> str:
  ...
  const chimeOptions = [
    ...
    { key: 'beep_single_high', title: 'Beep · Single High' },
    { key: 'beep_single_low', title: 'Beep · Single Low' },
    { key: 'beep_double', title: 'Beep · Double' },
...
let useDerivedPresence = false;
const PRESENCE_HANG_MS = 3000;
const MAX_TRACKS = 2;
let radarLastSeenMs = 0;
let radarLastSeenDistanceCm = 0;
let radarRawPresent = false;
let radarDerivedPresent = false;
let nextTrackId = 1;
let tracks = [];
...
function updateCameraPanel(status) {
  ...
  if (status && typeof status === 'object') {
    ...
    connected = String(status || '').toLowerCase() === 'connected';
  }
  const normalized = connected ? 'connected' : 'disconnected';
...
function renderCameraMeta(meta) {
  const metaEl = document.getElementById('home2CameraMeta');
  if (!metaEl) return;
  if (!meta || !meta.ok) {
    metaEl.textContent = 'Latest: n/a';
    return;
  }
  const ts = String(meta.ts || '');
  const reason = String(meta.reason || 'unknown');
  const tsText = ts ? relativeAge(ts) : 'n/a';
  metaEl.textContent = 'Latest: ' + tsText + ' · reason: ' + reason;
}

async function refreshCameraLatestMeta() {
  try {
    const resp = await fetch('/camera/latest/meta', { cache: 'no-store' });
    if (!resp.ok) {
...
function bindCameraControls() {
  ...
  const triggerBtn = document.getElementById('home2CameraTriggerBtn');
  if (triggerBtn && triggerBtn.dataset.bound !== '1') {
    triggerBtn.dataset.bound = '1';
    triggerBtn.addEventListener('click', async (event) => {
      event.preventDefault();
      triggerBtn.disabled = true;
      try {
        const result = await postJson('/camera/trigger', { reason: 'manual' });
        renderCameraMeta(result || null);
      } catch (_err) {
      } finally {
  ...
  for (const imageId of imageIds) {
    const imgEl = document.getElementById(imageId);
    if (!imgEl || imgEl.dataset.bound === '1') continue;
...
function applyHome2Layout(layout) {
  ...
  for (const item of layout) {
    ...
    const normalizedId = String(item.id) === 'vision-card' ? 'camera-card' : String(item.id);
    const el = document.querySelector('.home2-card[data-card-id="' + normalizedId + '"]');
    if (!el) continue;
...
function defaultHome2Layout() {
  return [
    ...
    { id: 'camera-card', x: 0, y: 16, w: 6, h: 8 },
...
function updatePresenceState(frame) {
  const now = Date.now();
  const target = Number(frame && frame.target ? frame.target : 0);
  const detectCm = Number(frame && frame.detect_cm ? frame.detect_cm : 0);
  radarRawPresent = target > 0 || detectCm > 0;
  if (radarRawPresent) {
    radarLastSeenMs = now;
    radarDerivedPresent = true;
    if (detectCm > 0) {
      radarLastSeenDistanceCm = detectCm;
    }
  } else {
    radarDerivedPresent = (now - radarLastSeenMs) < PRESENCE_HANG_MS;
  }
  return radarDerivedPresent;
}
...
function computeConfidence(distanceMeters) {
  if (distanceMeters <= 0.8) return { label: 'Very High', score: 0.9, css: 'conf-very-high' };
  if (distanceMeters <= 1.5) return { label: 'High', score: 0.75, css: 'conf-high' };
  if (distanceMeters <= 3.0) return { label: 'Moderate', score: 0.6, css: 'conf-moderate' };
  if (distanceMeters <= 5.0) return { label: 'Low', score: 0.4, css: 'conf-low' };
...
function updateSemanticDistanceBar(distanceCm, visible, alive) {
  ...
  const showTarget = !!alive && !!visible;
...
function distanceGateMeters(d) {
  if (d <= 1.5) return 0.25;
  if (d <= 3.0) return 0.4;
  return 0.6;
}

function hangVisible(lastSeenMs) {
  if (!lastSeenMs) return { visible: false, fade: 0 };
  const age = Date.now() - lastSeenMs;
  if (age >= PRESENCE_HANG_MS) return { visible: false, fade: 0 };
  const fade = 1 - Math.min(age / PRESENCE_HANG_MS, 1);
  return { visible: true, fade };
}

function getDetectionsFromFrame(frame) {
  const dets = [];
  const moveCm = Number(frame && frame.move_cm ? frame.move_cm : 0);
  const stillCm = Number(frame && frame.stat_cm ? frame.stat_cm : 0);
  if (moveCm > 0) {
    dets.push({ kind: 'move', distanceM: (moveCm / 100.0), move_en: Number(frame.move_en || 0), stat_en: Number(frame.stat_en || 0) });
  }
  if (stillCm > 0) {
    dets.push({ kind: 'still', distanceM: (stillCm / 100.0), move_en: Number(frame.move_en || 0), stat_en: Number(frame.stat_en || 0) });
  }
  return dets;
}
...
function updateTracksFromDetections(detections) {
  ...
  for (const det of detections) {
    ...
    for (const track of tracks) {
      ...
      const err = Math.abs(Number(track.distanceM || 0) - Number(det.distanceM || 0));
      const gate = distanceGateMeters(Number(det.distanceM || 0));
      if (err <= gate && err < bestErr) {
        best = track;
        bestErr = err;
      }
    }
  ...
  tracks = tracks.filter((track) => (now - Number(track.lastSeenMs || 0)) < PRESENCE_HANG_MS);
  tracks.sort((a, b) => Number(a.distanceM || 0) - Number(b.distanceM || 0));
...
function renderTrackMarkers(maxM, listOverride) {
  ...
  for (let i = 0; i < list.length; i += 1) {
    ...
    const clamped = Math.max(0, Math.min(Number(track.distanceM || 0), maxM));
...
function renderSonarCallout(activeTrack, dotMeta, maxM) {
  ...
  const mvValEl = document.getElementById('radar-float-move-val');
...
function updateRadarReadout(state) {
  ...
  const frame = {
    ...
    detect_cm: Number(state.detect_cm || 0),
    move_cm: Number(state.move_cm || 0),
    stat_cm: Number(state.stat_cm || 0),
    move_en: moveMetric,
    stat_en: statMetric,
  };
  const detections = getDetectionsFromFrame(frame);
  if (alive) {
  ...
  if (lastSeenEl) {
    if (activeTrack && activeTrack.lastSeenMs) {
      const ageSec = ((Date.now() - Number(activeTrack.lastSeenMs || 0)) / 1000).toFixed(1);
      lastSeenEl.textContent = 'Last seen: ' + ageSec + 's ago';
    } else {
  ...
  if (isHome2Layout()) {
    ...
    cameraCard.innerHTML =
      ...
      '<div class="camera-controls">' +
        '<button id="home2CameraStartBtn" class="btn" onclick="startCameraStream()">Start Stream</button>' +
        '<button id="home2CameraStopBtn" class="btn" onclick="stopCameraStream()">Stop Stream</button>' +
        '<a id="home2CameraSnapshotBtn" class="btn" href="/camera/snapshot" target="_blank" rel="noopener">Take Snapshot</a>' +
        '<button id="home2CameraTriggerBtn" class="btn">Trigger Snapshot</button>' +
      '</div>' +
      '<div id="home2CameraMeta" class="muted small">Latest: n/a</div>' +
...
async function pollStatus() {
  ...
  try {
    ...
    updateCameraPanel(health.camera || { connected: false });
    setLastUpdatedNow();
  } catch (err) {
    if (!(err instanceof DOMException && err.name === 'AbortError')) {
      console.error(err);
